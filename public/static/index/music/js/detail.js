;(function($){
	...
	var rotate_id=-1;//记录图片旋转的循环id
	var rotate_deg=0;//记录图片的旋转角度
	var config = {
	...
	window[fnName] = function(setConfig){
		...
		_this.playAudio = function(){
			...
			rotate_id=setInterval(function(){
				rotate_deg=(rotate_deg+1)%360;
				var rotate_img=document.getElementsByClassName("img-circle");
				for(var i=0;i<rotate_img.length;i++){
					rotate_img[i].style.transform="rotate("+rotate_deg+"deg)";
				}
		...
		// prev.on('click',function(){
			...
			// _this.selectMenu(songEq,true);
			// var url="/public/index/music/detail.html?id="+id;
		// });
